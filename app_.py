...
if HF_TOKEN:
    try:
        ...
        st.warning(f"⚠ HuggingFace client could not initialize: {e}")
else:
    st.warning("⚠ HuggingFace token not set. AI Health Advisor will be disabled.")
...
def llm_explanation(outcome, risk_score, glucose, blood_pressure, skin_thickness, insulin, bmi, dpf, age):
    if client is None:
        return "⚠ AI Advisor disabled: Missing HuggingFace token. Set HF_TOKEN environment variable."
...
Prediction:
- {outcome} with {risk_score}% score

Explain this in structured medical reasoning + lifestyle advice.
"""
    try:
